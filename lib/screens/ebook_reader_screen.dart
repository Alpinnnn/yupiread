...
class _EbookReaderScreenState extends State<EbookReaderScreen>
  ...
  static const Duration _panThrottle = Duration(
    milliseconds: 33,
  ); // 30fps for pan
  ...
  void _updateProgress(int pageNumber) {
    ...
    if (updatedEbook != null && updatedEbook.progress != _ebook?.progress) {
      ...
      print(
        'DEBUG: Progress check - oldProgress: $oldProgress, newProgress: $newProgress, totalPages: ${updatedEbook.totalPages}, currentPage: ${updatedEbook.currentPage}',
      );
      ...
      } else if (updatedEbook.totalPages > 1 &&
          oldProgress < 1.0 &&
          newProgress >= 1.0) {
      ...
      if (shouldLogCompletion) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              '🎉 Selamat! Ebook "${updatedEbook.title}" telah selesai dibaca!',
            ),
  ...
  void _showEditPdfDialog() {
    ...
    List<String> selectedTags = List.from(_ebook!.tags);
                    ...
                    ElevatedButton(
                      onPressed: () {
                        ...
                        if (newTitle.isNotEmpty) {
                          ...
                          setState(() {
                            _ebook = _dataService.getEbook(widget.ebookId);
                          });
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                              content: Text('PDF berhasil diperbarui'),
  ...
  Future<void> _sharePdf() async {
    ...
    // Close bottom bar after sharing
    setState(() {
      _showBottomBar = false;
    });
  }
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              Text('Masukkan nomor halaman (1-$_totalPages):'),
              const SizedBox(height: 16),
              TextField(
  ...
  void _jumpToPage(String pageText) {
    final int? targetPage = int.tryParse(pageText);
    if (targetPage != null && targetPage >= 1 && targetPage <= _totalPages) {
      _pdfViewerController.jumpToPage(targetPage);
      setState(() {
        _currentPage = targetPage;
        if (targetPage > _maxPageReached) {
          _maxPageReached = targetPage;
        }
      });
      _updateProgress(targetPage);
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Nomor halaman harus antara 1 dan $_totalPages'),
          backgroundColor: const Color(0xFFEF4444),
        ),
      );
    }
              ...
              children: [
                Listener(
                  ...
                  child: GestureDetector(
                    ...
                    onDoubleTapDown: _showBottomBar ? null : _handleDoubleTap,
                    child: Container(
                      ...
                      child: GestureDetector(
                        ...
                        onScaleUpdate: (details) {
                          ...
                          if (now.difference(_lastGestureUpdate) <
                              _gestureThrottle) {
                            return;
                          }
                          ...
                          if (details.scale != 1.0) {
                            ...
                            if ((newZoomLevel - _currentZoomLevel).abs() >
                                0.05) {
                              setState(() {
                                _currentZoomLevel = newZoomLevel;
                          ...
                          if (_currentZoomLevel > 1.0 &&
                            ...
                            if (panNow.difference(_lastPanUpdate) >=
                                _panThrottle) {
                        ...
                        child: Transform.translate(
                          ...
                          child: SfPdfViewer.file(
                            ...
                            onZoomLevelChanged: (PdfZoomDetails details) {
                              ...
                              if (now.difference(_lastZoomUpdate) <
                                  _zoomThrottle) {
                                return;
                              }
                              _lastZoomUpdate = now;
